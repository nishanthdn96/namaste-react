...
const RestaurantCards = ({ resObj }) => {
  ...
  return (
    <div className="m-4 p-4 w-[300px] rounded-lg min-h-[400px] bg-gray-100 hover:bg-gray-200">
      <img
        className="rounded-lg w-[260px] h-[200px]"
        alt="res-logo"
        src={CDN_URL + cloudinaryImageId}
      ></img>
      <h4 className="font-bold py-2 text-lg">{restroName}</h4>
      <h4 className="font-extralight py-2">{costForTwo}</h4>
      <h4 className="font-thin py-2">{truncatedCuisine}</h4>
      <h4>{avgRating}</h4>
    </div>
  );
};
...
export const offerCard = (RestaurantCard) => {
  return (props) => {
    return (
      <div>
        <label className="absolute bg-black text-white mx-1 p-[1px] rounded-lg">
